...
function parseDocType(doc, html) {
  if (html.match(DOC_HTML5)) {
    return doc.implementation.createDocumentType("html", "", "");
  }

  var start = html.indexOf(DOC_TYPE_START);
  var end = html.indexOf(DOC_TYPE_END);
  if (start === -1 || end === -1) {
    return null;
  }

  var docString = html.substring(start, end + DOC_TYPE_END.length);
  var doctypeParsed = docString.replace(/[\n\r]/g, "").match(DOC_TYPE);
  if (!doctypeParsed) {
    return null;
  }
  var doctypePieces = doctypeParsed[1].split(" \"");

  var id1 = doctypePieces.length > 0 ? doctypePieces.pop().replace(/"/g, "") : "";
  var id2 = doctypePieces.length > 0 ? doctypePieces.pop().replace(/"/g, "") : "";
  var name = doctypePieces.length > 0 ? doctypePieces[0].split(" ")[0] : "html";
